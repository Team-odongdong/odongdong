...
@Service
public class ChangeByGeocoder {

    @Value("${kakao.REST_API_KEY}")
    private String REST_API_KEY;

    public AddressInfoDto getCoordinateByAddress(String address) throws JSONException {
        String urlString = String.format("https://dapi.kakao.com/v2/local/search/address.json?analyze_type=similar&page=1&size=10&query=%s", address);
        WebClient webClient = WebClient.builder().baseUrl(urlString).defaultHeader("Authorization", "KakaoAK " + REST_API_KEY).build();
        String result = webClient.get().uri("").accept(MediaType.APPLICATION_JSON).retrieve().bodyToMono(String.class).block();
        JSONObject jsonObject = new JSONObject(result);
        if(jsonObject.getJSONArray("documents").isNull(0)) {
            AddressInfoDto nullAddressInfoDto = AddressInfoDto.builder().address(address).address_detail("").longitude(0.0).latitude(0.0).build();
    ...
    public CoordinateInfoDto getAddressByCoordinate(Double x, Double y) throws JSONException {
