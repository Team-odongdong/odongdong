...
public class QueryMaker {
	...
	public static String makeInsertQuery(List<Map<String, String>> insertTargetMap, DomainType domainType, Map<String, DatabaseType> databaseColumnTypeMap) {
		...
		insertQuery += insertTargetMap.stream().map((m) -> {
			String collect = "(";
			collect += set.stream().map((s) -> {
				String data = m.get(s);
